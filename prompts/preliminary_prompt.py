...
deck, resume, or video) — used as soon as that material finishes Layer 1/2
analysis, before the rest of the session's materials exist yet. Reuses the
same per-material section builders as `evaluation_prompt.py` (no prompt
text is duplicated), just with a narrower task list and an explicit
instruction not to reference materials that have not been uploaded yet.

The FINAL synthesis prompt (`evaluation_prompt.build_evaluation_prompt`)
later receives these preliminary `ReasoningPayload`s as additional context
so the final report reconciles them rather than reasoning from scratch.
"""
...
def build_preliminary_prompt(
    ...
    Args:
        stage: One of "slide", "resume", "video" — which material this
            preliminary pass covers. `features` should only contain the
            fields relevant to that material (see `db.models.EvaluationStage`
            and `services/workflow_manager.py`'s preliminary-evaluation
            methods, which build a narrowed `UnifiedFeatureModel` per stage).
    ...
    feedback_field = "presentation_feedback" if stage in ("slide", "video") else "interview_feedback"
...
candidate has submitted so far. No other material has been uploaded yet —
do not mention, assume the existence of, or apologize for the absence of
any material other than what is shown below. This is an early checkpoint,
not the final report; a more complete, synthesized report will be produced
once every material has been submitted.
